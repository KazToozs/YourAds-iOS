//
//  YourAdsSDK.h
//  YourAdsSDK
//
//  Created by Cris Toozs on 19/11/2017.
//  Copyright © 2017 Cris Toozs. All rights reserved.
//

#import <YourAdsSDK/OpenCVWrapper.h>

//! Project version number for YourAdsSDK.
FOUNDATION_EXPORT double YourAdsSDKVersionNumber;

//! Project version string for YourAdsSDK.
FOUNDATION_EXPORT const unsigned char YourAdsSDKVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <YourAdsSDK/PublicHeader.h>


